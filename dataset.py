    ...
    def _get_single_item(self, idx, image_height):
        ...
        if self.return_dict:
            return {
                ...
                'coords': coords,
                'rgb_files': str(self.rgb_files[idx]),
            }
...
class ConcatCamLocDataset(ConcatDataset):
    def __init__(self, dataset_list,
        ...
        for dataset in dataset_list:
            mean_cam_center += dataset.mean_cam_center*len(dataset)
            sample_num += len(dataset)
        self.mean_cam_center = mean_cam_center/sample_num

    def __getitem_single(self, idx, image_height):
        if idx < 0:
            if -idx > len(self):
        ...
        data_pack = self.datasets[dataset_idx]._get_single_item(sample_idx, image_height)
        ...
        elif isinstance(data_pack, tuple):
            return data_pack+ (dataset_idx,)
        else:
            raise Exception("Wrong type of the data pack.")
    ...
    def __getitem__(self, idx):
        ...
        image_height = int(self.image_height * scale_factor)
        if type(idx) == list:
            tensors = [self.__getitem_single(i, image_height) for i in idx]
            return default_collate(tensors)
        else:
            return self.__getitem_single(idx, image_height)

if __name__ == "__main__":
    # scene_path="datasets/Urbanscape/drone_real/train"
    ...
    #         return_dict=True,
    #     )
    # batch_sampler = sampler.BatchSampler(sampler.RandomSampler(dataset),
    #                                          batch_size=16,
    #                                          drop_last=False)
    ...
    #     print(batch_data['rgb_files'][0])
    #     print(batch_data['pose'][0])
    #     if idx > 0:
    #         break

    # scene_path_list=[
    #     "datasets/7scenes/chess",
    #     "datasets/7scenes/fire",
    #     "datasets/7scenes/heads",
    # ]
    scene_path_list=[
    ...
    dataset_ensumble = ConcatCamLocDataset(datasets_list)
    batch_sampler = sampler.BatchSampler(
        sampler.RandomSampler(dataset_ensumble),
        batch_size=16,
        drop_last=False,
    )

    dataloader = torch.utils.data.DataLoader(
        dataset=dataset_ensumble,
        sampler=batch_sampler,
        batch_size=None,
        # shuffle=False,
        num_workers=8,
    ...
    print(dataset_ensumble.mean_cam_center)
    
    for idx, batch_data in enumerate(dataloader):
        print(idx, batch_data['dataset_id'][0])

    # for idx, batch_data in enumerate(dataloader):
    #     print(idx, batch_data[-1][0])