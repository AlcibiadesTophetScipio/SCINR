...
class Encoder(nn.Module):
    ...
    def __init__(self, out_channels=512, pool=True):
        ...
        self.res2_conv1 = nn.Conv2d(256, self.out_channels, 3, 1, 1)
        self.res2_conv2 = nn.Conv2d(self.out_channels, self.out_channels, 1, 1, 0)
        self.res2_conv3 = nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1)
...
class Head(nn.Module):
    ...
    def __init__(self,
        ...
        self.head_channels = head_channels  # Hardcoded.
...
class Regressor(nn.Module):
    ...
    @classmethod
    def create_from_encoder(cls, encoder_state_dict, mean, num_head_blocks, use_homogeneous, **kwargs):
        """
    ...
    @classmethod
    def create_from_state_dict(cls, state_dict, **kwargs):
        """
    ...
    def create_from_split_state_dict(cls, encoder_state_dict, head_state_dict, **kwargs):
        ...
        return cls.create_from_state_dict(merged_state_dict, **kwargs)
...
class ACENet(nn.Module):
    def __init__(
        ...
        if self.encoder_pth:
            encoder_state_dict = torch.load(self.encoder_pth, map_location="cpu")
            self.encoder.load_state_dict(encoder_state_dict)
        self.heads = Head(mean, num_head_blocks, use_homogeneous, 
                          in_channels=self.feature_dim,
                          head_channels=num_head_channels,
                          )

    def forward(self, inputs, flag='train'):
        """
        Forward pass.
        """
        ...
        if self.pixel_grid_2HW is None:
            self.pixel_grid_2HW = get_pixel_grid(self.downsample).permute(1,2,0).to(inputs.device)
        pixel_positions_hw2 = self.pixel_grid_2HW[:H, :W].clone()
        pixel_positions_bhw2 = pixel_positions_hw2.unsqueeze(0).expand(B,H,W,2)
        ...
        #     h,w = H//self.downsample, W//self.downsample
        # else:
        #     raise Exception('Wrong forward flag.')
        
        
        return self.get_scene_coordinates(features), pixel_positions_bhw2.permute(0,-1,1,2)
