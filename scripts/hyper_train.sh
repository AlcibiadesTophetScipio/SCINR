...
function print_results {
  for scene in "${scenes[@]}"; do
    ...
    if [ $3 == 'sep' ]; then
      ...
      echo "${scene_mod}: $(cat "${out_dir}/log_test_${scene_mod}.txt" | tail -$1 | head -1)"
    else
...
training_exe="${REPO_PATH}/trainer_hyper.py"
...
if [ "$3" == "together" ]; then
  out_dir="${REPO_PATH}/output-Hyper/$2-${head_mod}-together"
  mkdir -p "$out_dir"
  if [ "$5" == "print" ]; then
  ...
  for scene in "${scenes[@]}"; do
    ...
    python $testing_exe \
           ...
           --regressor_select "base" \
           --head_mod "$head_mod" \
           2>&1 | tee "${out_dir}/log_test_${scene_mod}.txt"
  done
  
elif [ "$3" == "sep" ]; then
  out_dir="${REPO_PATH}/output-Hyper/$2-${head_mod}-sep"
  for scene in "${scenes[@]}"; do
    scene_mod=${scene#"$2/"}
    scene_dir=${out_dir}/$scene_mod
    mkdir -p "$scene_dir"
