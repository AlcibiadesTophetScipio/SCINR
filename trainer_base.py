...
from dataset import CamLocDataset, ConcatCamLocDataset
...
class TrainerACE:
    def __init__(self, options):
        ...
        # Create dataset.
        datasets_list=[]
        for scene_path in self.options.scenes:
            datasets_list.append(
                CamLocDataset(
                    root_dir= Path(scene_path)/ "train",
        ...
        self.dataset = ConcatCamLocDataset(
            datasets_list,
            image_height=self.options.image_resolution,
            augment=self.options.use_aug,
            aug_scale_max=self.options.aug_scale,
        ...
        self.regressor = options.Regressor.create_from_encoder(
            encoder_state_dict,
    ...
    def train(self):
        ...
        _logger.info(self.regressor)
        model_train_params = get_parameter_num(self.regressor)["trainable"]
        _logger.info(f'Trained params: {model_train_params/1024**2:.4f} M.')
        head_params = get_parameter_num(self.regressor, 'heads')
        _logger.info(f'Head params: {head_params/1024**2:.4f} M.')
    ...
    def create_training_buffer(self):
        ...
        self.training_buffer = {
            ...
            'intrinsics_inv': torch.empty((self.options.training_buffer_size, 3, 3), dtype=torch.float32,
                                          device=self.device),
            'dataset_id': torch.empty((self.options.training_buffer_size, 1), dtype=torch.int, device=self.device),
            'norm_pos': torch.empty((self.options.training_buffer_size, 2), dtype=torch.float32, device=self.device),
        }
        ...
        with torch.no_grad():
            ...
            while buffer_idx < self.options.training_buffer_size:
                ...
                for pack_data in training_dataloader:
                    # Copy to device.
                    image_B1HW = pack_data['image'].to(self.device, non_blocking=True)
                    image_mask_B1HW = pack_data['image_mask'].to(self.device, non_blocking=True)
                    gt_pose_inv_B44 = pack_data['pose_inv'].to(self.device, non_blocking=True)
                    intrinsics_B33 = pack_data['intrinsics'].to(self.device, non_blocking=True)
                    intrinsics_inv_B33 = pack_data['intrinsics_inv'].to(self.device, non_blocking=True)
                    dataset_id_B = pack_data['dataset_id'].to(self.device, non_blocking=True)
                    ...
                    norm_pos_B2HW = get_norm_grid([H,W]).reshape(1,2,H,W).expand_as(pixel_positions_B2HW).to(self.device, non_blocking=True)
                    ...
                    dataset_id = dataset_id_B.unsqueeze(1).expand(B, H * W).reshape(-1, 1)
                    ...
                    batch_data = {
                        ...
                        'norm_pos': normalize_shape(norm_pos_B2HW),
                    }
    ...
    def run_epoch(self):
        ...
        for batch_start in range(0, self.options.training_buffer_size, self.options.batch_size):
            ...
            self.training_step(
                ...
                self.training_buffer['dataset_id'][random_batch_indices].contiguous(),
                self.training_buffer['norm_pos'][random_batch_indices].contiguous(),
            )
            self.iteration += 1
    ...
    def training_step(self, features_bC, target_px_b2, gt_inv_poses_b34, Ks_b33, invKs_b33, dataset_id_b1, norm_pos_b2):
        ...
        if self.iteration % self.iterations_output == 0:
            ...
            _logger.info(f'Iteration: {self.iteration:6d} / Epoch {self.epoch:03d}|{self.options.epochs:03d}, lr: {self.optimizer.param_groups[0]["lr"]}, '
                         f'Loss: {loss:.2f}, Valid: {fraction_valid * 100:.2f}%, Cos_v: {cos_v:.2f}, Cos_inv: {cos_inv:.2f}, Time: {time_since_start:.2f}s')
