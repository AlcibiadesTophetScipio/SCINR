...
from utils import xyz_to_rgb, calc_proj, get_pixel_grid, get_norm_grid
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene', type=Path,
                        default= Path(__file__).parent / 'datasets/Cambridge/GreatCourt',
                        help='path to a scene in the dataset folder, e.g. "datasets/Cambridge/GreatCourt"')

    parser.add_argument('--network', type=Path, 
                        default= Path(__file__).parent /'output/temp/cambridge.pt',
    ...
    parser.add_argument('--regressor_select', type=str, default="base",
                        choices=['ace', 'base'],
                        help='Head model selection.')
    ...
    dataset_id = 0
    if "GreatCourt" in str(opt.scene):
        dataset_id = 0
    elif "KingsCollege" in str(opt.scene):
        dataset_id = 1
    elif "OldHospital" in str(opt.scene):
        dataset_id = 2
    elif "ShopFacade" in str(opt.scene):
        dataset_id = 3
    elif "StMarysChurch" in str(opt.scene):
        dataset_id = 4
    elif "chess" == opt.scene.stem:
        dataset_id = 0
    elif "fire" == opt.scene.stem:
        dataset_id = 1
    elif "heads" == opt.scene.stem:
        dataset_id = 2
    elif "office" == opt.scene.stem:
        dataset_id = 3
    elif "pumpkin" == opt.scene.stem:
        dataset_id = 4
    elif "redkitchen" == opt.scene.stem:
        dataset_id = 5
    elif "stairs" == opt.scene.stem:
        dataset_id = 6
    elif "apt1_kitchen" == opt.scene.stem:
        dataset_id = 0
    elif "apt1_living" == opt.scene.stem:
        dataset_id = 1
    elif "apt2_bed" == opt.scene.stem:
        dataset_id = 2
    elif "apt2_kitchen" == opt.scene.stem:
        dataset_id = 3
    elif "apt2_living" == opt.scene.stem:
        dataset_id = 4
    elif "apt2_luke" == opt.scene.stem:
        dataset_id = 5
    elif "office1_gates362" == opt.scene.stem:
        dataset_id = 6
    elif "office1_gates381" == opt.scene.stem:
        dataset_id = 7
    elif "office1_lounge" == opt.scene.stem:
        dataset_id = 8
    elif "office1_manolis" == opt.scene.stem:
        dataset_id = 9
    elif "office2_5a" == opt.scene.stem:
        dataset_id = 10
    elif "office2_5b" == opt.scene.stem:
        dataset_id = 11
    else:
        raise Exception("Wrong dataset name.")
    
    if opt.dataset_id != -1:
        dataset_id = torch.tensor(opt.dataset_id).to(device)
    else:
        dataset_id = torch.tensor(dataset_id).to(device)
    ...
    SRegressor = None
    if opt.regressor_select == 'ace':
        SRegressor = ACENet
    elif opt.regressor_select == 'base':
        SRegressor = BaseRegressor
    else:
        raise Exception("Regressor error!")
    
    # Create regressor.
    network = SRegressor.create_from_split_state_dict(
        encoder_state_dict, 
        head_state_dict,
        Head=heads_repo[opt.head_mod],
    )
    ...
    pixel_grid_2HW = get_pixel_grid(8)
    # Testing loop.
    testing_start_time = time.time()
    with torch.no_grad():
        for image_B1HW, _, gt_pose_B44, gt_pose_inv_B44, intrinsics_B33, _, _, filenames in testset_loader:
            ...
            with autocast(enabled=True):
                ...
                norm_pos_B2HW = get_norm_grid([H,W]).reshape(1,2,H,W).to(device)
                dataset_id = dataset_id.expand([1,1, *features_BCHW.shape[-2:]])
                scene_coordinates_B3HW = network.get_scene_coordinates(
                    features_BCHW,
            ...
            pred_px_B2HW = calc_proj(
                pred_scene_coords_B3HW=scene_coordinates_B3HW,
                gt_pose_inv_B44=gt_pose_inv_B44,
            ...
            target_px_B2HW = pixel_positions_B2HW.expand(B, 2, H, W)

            reprojection_error_B2HW = pred_px_B2HW - target_px_B2HW.cpu()
            reprojection_error_BHW = torch.norm(reprojection_error_B2HW, dim=1, p=1)

            # Each frame is processed independently.
            for frame_idx, (scene_coordinates_3HW, gt_pose_44, intrinsics_33, frame_path, reprojection_error_HW) in enumerate(
                    zip(scene_coordinates_B3HW, gt_pose_B44, intrinsics_B33, filenames, reprojection_error_BHW)):
